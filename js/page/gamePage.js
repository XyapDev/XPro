...
<span id="serverFilters" class="rbx-selection-label ng-binding" style="width:160px;overflow:hidden;">Server Filters<img class="server-filters-icon" style="width:20px;filter:invert(2);margin-left:5px;" src="${chrome.runtime.getURL('/images/serverfilters.png')}"></span> 
</button>
</div>
...
function addMostRecentServer(placeID, serverID, userID, time, serverActive) {
	...
	if (serverActive) {
		...
		<div style="float:left;" class="text-info rbx-game-status rbx-game-server-status"><b>Last Played:</b><img style="background-image:none;margin:0px;margin-top:-2px;margin-bottom:1px;transform:scale(1);border:none;margin-left:5px;margin-right:5px;width:15px;height:15px;" src="${chrome.runtime.getURL(`/images/timer${theme == "dark" ? "_dark" : "_light"}.svg`)}" class="info-label icon-pastname">${stripTags(timeString)}</div><br><div style="float:left;" class="text-info rbx-game-status rbx-game-server-status"><b>Server ID:</b> ${stripTags(serverID)}</div>
